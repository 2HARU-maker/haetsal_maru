...
const smokeRunner=`<script>(function(){try{window.HaetsalAdminAuth={isVerified:()=>true};window.HaetsalManagerAuth={isVerified:()=>true};const results=[];const capture=(label)=>({label,view:window.OrderModule&&window.OrderModule.state&&window.OrderModule.state.currentView,role:window.OrderModule&&window.OrderModule.state&&window.OrderModule.state.appRole,tab:window.OrderModule&&window.OrderModule.state&&window.OrderModule.state.dashboardTab,html:document.body.innerHTML.length,scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth});const boot={order:!!window.OrderModule,tumbler:!!window.TumblerModule,main:!!document.getElementById('main-screen'),orderScreen:!!document.getElementById('order-screen')};if(!boot.order||!boot.tumbler||!boot.main||!boot.orderScreen) throw new Error('boot objects missing');window.HaetsalOperatorSession.setRole('ADMIN');window.OrderModule.openDashboard('ADMIN','ORDERS');for(const tab of ['ORDERS','SUMMARY','STATS','TUMBLER','USERS','MILK_SUPPORT','SETTINGS','STABILITY']){window.OrderModule.state.dashboardTab=tab;window.OrderModule.render();results.push(capture('ADMIN_'+tab))}window.HaetsalOperatorSession.setRole('MANAGER');window.OrderModule.openDashboard('MANAGER','ORDERS');for(const tab of ['ORDERS','SUMMARY','STATS','TUMBLER','USERS','MILK_SUPPORT','SETTINGS']){window.OrderModule.state.dashboardTab=tab;window.OrderModule.render();results.push(capture('MANAGER_'+tab))}window.OrderModule.state.appRole='GUEST';window.OrderModule.state.currentView='ORDER_MAIN';window.OrderModule.render();results.push(capture('ORDER_MAIN'));const payload={ok:true,boot,results,errors:window.__patch34Errors.slice(),innerWidth:innerWidth,innerHeight:innerHeight};const m=document.createElement('meta');m.id='patch34-smoke-result';m.setAttribute('data-json',encodeURIComponent(JSON.stringify(payload)));document.head.appendChild(m)}catch(error){const m=document.createElement('meta');m.id='patch34-smoke-result';m.setAttribute('data-json',encodeURIComponent(JSON.stringify({ok:false,error:String(error&&error.stack||error),errors:(window.__patch34Errors||[]).slice()})));document.head.appendChild(m)}})();<\/script>`;
