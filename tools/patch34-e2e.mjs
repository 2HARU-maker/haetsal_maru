...
const smokeRunner=`<script>(function(){try{window.HaetsalAdminAuth={isVerified:()=>true};window.HaetsalOperatorSession={isActive:()=>true,getRole:()=> 'ADMIN',setRole:()=>true};const results=[];const capture=(label)=>({label,view:window.OrderModule&&window.OrderModule.state&&window.OrderModule.state.currentView,tab:window.OrderModule&&window.OrderModule.state&&window.OrderModule.state.dashboardTab,html:document.body.innerHTML.length,scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth});const boot={order:!!window.OrderModule,tumbler:!!window.TumblerModule,main:!!document.getElementById('main-screen'),orderScreen:!!document.getElementById('order-screen')};if(!boot.order||!boot.tumbler||!boot.main||!boot.orderScreen) throw new Error('boot objects missing');window.OrderModule.openDashboard('ADMIN','ORDERS');results.push(capture('ORDERS'));window.OrderModule.state.dashboardTab='SUMMARY';window.OrderModule.render();results.push(capture('SUMMARY'));window.OrderModule.state.dashboardTab='TUMBLER';window.OrderModule.render();results.push(capture('TUMBLER'));window.OrderModule.state.appRole='GUEST';window.OrderModule.state.currentView='ORDER_MAIN';window.OrderModule.render();results.push(capture('ORDER_MAIN'));const payload={ok:true,boot,results,errors:window.__patch34Errors.slice(),innerWidth:innerWidth,innerHeight:innerHeight};const m=document.createElement('meta');m.id='patch34-smoke-result';m.setAttribute('data-json',encodeURIComponent(JSON.stringify(payload)));document.head.appendChild(m)}catch(error){const m=document.createElement('meta');m.id='patch34-smoke-result';m.setAttribute('data-json',encodeURIComponent(JSON.stringify({ok:false,error:String(error&&error.stack||error),errors:(window.__patch34Errors||[]).slice()})));document.head.appendChild(m)}})();<\/script>`;
smokeHtml=smokeHtml.replace('</body>',smokeRunner+'</body>');
...
const py=spawn('python3',['-m','http.server','18080','--bind','127.0.0.1','--directory',root],{stdio:'ignore'});await new Promise(r=>setTimeout(r,300));
const smokeResults=[];
try{
