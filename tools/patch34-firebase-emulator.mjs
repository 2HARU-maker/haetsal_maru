...
try{
 ...
 dom=execFileSync(chrome,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--no-first-run','--no-default-browser-check','--window-size=1024,768',`--user-data-dir=${profile}`,'--virtual-time-budget=20000','--dump-dom','http://127.0.0.1:18081/'],{encoding:'utf8',maxBuffer:30*1024*1024,timeout:50000,stdio:['ignore','pipe','pipe']});
}finally{py.kill('SIGTERM')}
const match=dom.match(/<meta id="patch34-emulator-result" data-json="([^"]*)"/i);if(!match){console.log('EMULATOR_DOM_HEAD',dom.slice(0,3000));console.log('EMULATOR_DOM_TAIL',dom.slice(-8000));throw new Error('emulator browser result missing')}
